...
Test coverage is minimal — Vitest (`vitest.config.ts`, `npm test`) is wired up but only `src/lib/youtube.test.ts` exists so far; add `*.test.ts(x)` files alongside new pure-logic modules rather than inventing a separate test runner. `.github/workflows/ci.yml` runs lint + test + build on push/PR to `main`. After a change, validation is: `npm run lint`, `npm test`, `npm run build`, and manual verification via `npm run dev`.
