...
-- Create demo parent user and associate with student
...
SELECT 
  ...
  'parent'
FROM auth.users au 
WHERE au.email = 'parent@riverside.edu'
AND NOT EXISTS (
...
JOIN profiles student_profile ON student_profile.parent_email = 'parent@riverside.edu'
WHERE parent_profile.role = 'parent' 
  AND parent_profile.first_name = 'Sarah'
  AND parent_profile.last_name = 'Thompson'
